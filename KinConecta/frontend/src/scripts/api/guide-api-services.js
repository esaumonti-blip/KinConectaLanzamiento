(function () {
  ...
  async function categoryIdByName(name) {
    ...
    const found = categories.find((item) => String(item.name || "").trim().toLowerCase() === normalized);
  ...
  const tours = {
    ...
    async update(value, tourId, payload) {
      ...
      const response = await api.put(`/tours/${encodeURIComponent(tourId)}`, {
        ...
        coverImageUrl: payload?.coverImageUrl ?? current.coverImageUrl,
        imageClass: payload?.imageClass ?? current.imageClass,
